...
from PySide6.QtCore import Qt, QDate, Signal, Slot, QThread, QUrl, QTimer # Import QTimer
...
class MainWindow(QMainWindow):
    ...
    def setup_menu_bar(self):
        ...
        # Options menu (for refresh interval)
        options_menu = menu_bar.addMenu("Options")
        set_refresh_interval_action = QAction("Set Refresh Interval...", self)
        set_refresh_interval_action.triggered.connect(self.show_refresh_interval_dialog)
        options_menu.addAction(set_refresh_interval_action)
    ...
    def show_refresh_interval_dialog(self):
        """Show dialog to set refresh interval"""
        dialog = QDialog(self)
        dialog.setWindowTitle("Set Refresh Interval")
        dialog.setMinimumWidth(250)
        layout = QFormLayout(dialog)

        interval_spin = QSpinBox()
        interval_spin.setRange(1, 60) # 1 to 60 minutes
        interval_spin.setValue(self.refresh_interval_minutes)
        layout.addRow("Refresh every (minutes):", interval_spin)
    ...
    def start_auto_refresh(self):
        ...
        self.auto_refresh_timer.start(self.refresh_interval_minutes * 60 * 1000)
